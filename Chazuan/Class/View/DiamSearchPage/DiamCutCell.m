...
@interface DiamCutCell ()

/// viewModel
@property (nonatomic, readwrite, strong) CutItemViewModel *viewModel;

@property (nonatomic, readwrite, strong) NSArray *titleArr;
@property (nonatomic, readwrite, strong) NSMutableArray *btns;

@property (nonatomic, readwrite, strong) UILabel *threeLabel;
...
@implementation DiamCutCell

#pragma mark - Public Method
+ (instancetype)cellWithTableView:(UITableView *)tableView style:(UITableViewCellStyle)style {
    static NSString *ID = @"DiamCutCell";
    DiamCutCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) cell = [[self alloc] initWithStyle:style reuseIdentifier:ID];;
    return cell;
}

- (void)bindViewModel:(CutItemViewModel *)viewModel {
...
- (void)_setup {
    ...
    self.titleArr = @[@"3EX",@"3VG",@"3GD",@"3FR",@"EX",@"VG",@"GD",@"FR",@"EX",@"VG",@"GD",@"FR",@"EX",@"VG",@"GD",@"FR"];
    self.btns = [NSMutableArray array];
}

- (void)_setupSubviews {
    UILabel *threeLabel = [[UILabel alloc] init];
    threeLabel.font = kFont(15);
    threeLabel.textColor = kHexColor(@"#1C2B36");
    threeLabel.text = @"三项";
    self.threeLabel = threeLabel;
    [self.contentView addSubview:threeLabel];
    
    UILabel *cutLabel = [[UILabel alloc] init];
    cutLabel.font = kFont(15);
...
- (void)_setupSubviewsConstraint {
    ...
    [self.cutLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.contentView).offset(ZGCConvertToPx(10));
        make.size.mas_equalTo(self.threeLabel);
    }];
    [self.polishLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.cutLabel.mas_bottom);
        make.left.mas_equalTo(self.contentView).offset(ZGCConvertToPx(10));
        make.size.mas_equalTo(self.threeLabel);
    }];
