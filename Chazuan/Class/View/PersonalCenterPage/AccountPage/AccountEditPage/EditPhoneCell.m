...
// viewModel
@property (nonatomic, readwrite, strong) EditPhoneItemViewModel *viewModel;
...
+ (instancetype)cellWithTableView:(UITableView *)tableView style:(UITableViewCellStyle)style {
    static NSString *ID = @"EditPhoneCell";
    EditPhoneCell *cell = [tableView dequeueReusableCellWithIdentifier:ID];
    if (!cell) cell = [[self alloc] initWithStyle:style reuseIdentifier:ID];;
    return cell;
}

- (void)bindViewModel:(EditPhoneItemViewModel *)viewModel {
...
- (void)_setupSubviews {
    ...
    [textField zgc_limitMaxLength:11];
